...
export class WWActiveEffects {
  ...
  static async embedActiveEffects(actor, doc, operation = 'create') {
    ...
    switch (doc.type) {
      case 'ancestry':
        effectDataList = WWActiveEffects.generateEffectDataFromAncestry(doc, actor)
        break
      case 'path':
        effectDataList = WWActiveEffects.generateEffectDataFromPath(doc, actor)
        break
      case 'talent':
        effectDataList = WWActiveEffects.generateEffectDataFromTalent(doc, actor)
        break
      case 'armor':
        effectDataList = WWActiveEffects.generateEffectDataFromArmor(doc, actor)
        break
      default:
  ...
  static generateEffectDataFromAncestry(item, actor = null) {
    ...
    const effectDataL0 = {
      name: `${item.name} (${game.i18n.localize('WW.CharLevel')} 0)`,
      icon: item.img,
      origin: item.uuid,
      disabled: false,
      transfer: false,
    ...
    const effectDataL4 = {
      name: `${item.name} (${game.i18n.localize('WW.CharLevel')} 4)`,
      icon: item.img,
      origin: item.uuid,
  ...
  static generateEffectDataFromPath(item, actor = null) {
    ...
    pathdata.levels.forEach(pathLevel => {
      const levelEffectData = {
        name: `${item.name} (${game.i18n.localize('WW.CharLevel')} ${pathLevel.level})`,
        icon: item.img,
        origin: item.uuid,
  ...
  static async addEncumbrance(actor, itemNames) {
    ...
    const effectData = {
      name: effectName,
      icon: 'systems/weirdwizard/assets/icons/effects/fatigued.svg',
      origin: 'encumbrance',
      transfer: false,
      duration: { startTime: 0 },
      flags: {
