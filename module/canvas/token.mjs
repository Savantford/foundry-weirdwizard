...
export default class WWToken extends Token {
  ...
  _drawDamageBar(number, bar, data) {
    ...
    let colorPct = CONFIG.WW.IS_V12 ? Math.clamp(value, 0, max) / max : Math.clamped(value, 0, max) / max;
    if (game.settings.get('weirdwizard', 'damageBarReverse')) {
      colorPct = CONFIG.WW.IS_V12 ? Math.clamp(max-value, 0, max) / max : Math.clamped(max-value, 0, max) / max;
    }
    ...
    const stroke = CONFIG.WW.IS_V12 ? Math.clamp(h / 8, 1, 2) : Math.clamped(h / 8, 1, 2);
